...
Create a local environment with `python -m venv .venv`, then install dev dependencies with `.\.venv\Scripts\python -m pip install -e .[dev]`. Run the MCP server in default `stdio` mode with `.\.venv\Scripts\nethunt-mcp`. Execute the full test suite with `.\.venv\Scripts\python -m pytest`. Build the container with `docker build -t nethunt-mcp:latest .`. For HTTP transport smoke tests, use `docker compose up --build` and connect to `http://127.0.0.1:18044/mcp`.
...
## Agent & MCP Usage
For Codex work in this repository, prefer the local MCP server `nethunt-crm-local` configured in `~/.codex/config.toml` and backed by `http://127.0.0.1:18044/mcp`. Use it instead of the remote `nethunt-crm` entry when inspecting folders, fields, and records for local development. If it is unavailable, start the repo server from `.venv` with the `.env` values and override `MCP_TRANSPORT=streamable-http` rather than editing `.env`.
